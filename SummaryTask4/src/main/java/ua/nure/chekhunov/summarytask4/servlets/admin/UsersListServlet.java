package ua.nure.chekhunov.summarytask4.servlets.admin;
...
public class UsersListServlet extends HttpServlet {
    ...
    /**
     * Gets users list and if administrator sets some user
     * as new administrator, changes user's access level.
     * @param request
     * @param response
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
         ...
         //   int balance = DBManager.getInstance().getAllCostOrder(currentUser.getUserId());
            int counter = DBManager.getInstance().getAllCount(currentUser.getUserId());
            int page = 1;
            ...
            for(User user:usersList){
                balance = DBManager.getInstance().getAllCostOrder(user.getUserId());
                list.add(balance);
            }
        ...
        if (null != request.getParameter("actionName")) {
            ...
            if (act.equals("set_admin")) {
                ...
                try {
                    int res = DBManager.getInstance().setUserAdmin(userId);
                    if (res == 0) {
